...
const UploadMapping = () => {
  ...
  const handleReconcile = async () => {
    try {
      const res = await axios.post(`http://localhost:8080/reconcile/run`);
      setData(data.data);
    } catch (err) {
      debugger;
      console.error(err);
  ...
  const handleSystemFileChange = (e) => {
    const selectedSysFile = e.target.files[0];
    if (!selectedSysFile) return;

    const fileName = selectedSysFile.name.toLowerCase();
    ...
    if (selectedSysFile.size > maxSize) {
      alert("File size must be less than 50MB!");
      return;
    }

    setSysFile(selectedSysFile);

    //csv
    if (fileName.endsWith(".csv")) {
      Papa.parse(selectedSysFile, {
        header: true,
        skipEmptyLines: true,
        complete: function (results) {
          setSystemHeaders(Object.keys(results.data[0]));
          setSystemTableData(results.data.slice(0, 10));
        },
      });
    }
    ...
    //xlsx
      ...
      reader.onload = (e) => {
        ...
        setSystemHeaders(Object.keys(jsonData[0]));
        setSystemTableData(jsonData.slice(0, 10));
      };
      reader.readAsArrayBuffer(selectedSysFile);
    }
  };
  ...
  const handleSystemFileview = (e) => {
    ...
    if (fileName.endsWith(".csv")) {
      Papa.parse(file, {
        ...
        complete: function (results) {
          setSystemHeaders(Object.keys(results.data[0]));
          setSystemTableData(results.data.slice(0, 10)); // preview first 10 rows
        },
      });
    }
...
const handleSystemUpload = async () => {
  ...
  const formData = new FormData();
  formData.append("file", sysfile);

  try {
    setSysLoading(true);

    const response = await axios.post(
      "http://localhost:8080/api/systemupload",
      formData,
      {
        headers: {
          "Content-Type": "multipart/form-data",
    ...
    if (response.status === 200) {
      alert("System File uploaded successfully!");
      setSysFile(null);
    }
...
const handleFullReconciliation = async () => {
  // check both files selected
  if (!file || !sysfile) {
  ...
  try {
    ...
    systemFormData.append("file", sysfile);
    ...
    setData(reconcileResponse.data);
    ...
    alert(
      ...
      error.message ||
      "Error during reconciliation process"
    );
                ...
                <div className="bg-white dark:bg-slate-900 p-8 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                    ...
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm h-full">
                      ...
                      <div className="mt-6 p-4 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 flex items-start gap-3">
                        ...
                        <div>
                          ...
                          {sysfile && (
                            <p className="text-xs text-emerald-700 dark:text-emerald-400">
                              ({(sysfile.size / 1024).toFixed(2)} KB)
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                      ...
                      <div className="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-800">
                        <table className="w-full text-left border-collapse text-sm">
                          ...
                          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {systemTableData.map((row, i) => (
                              <tr
                                className="hover:bg-slate-50 dark:hover:bg-slate-800/50"
                                key={i}
                              >
                                {systemHeaders.map((h, j) => (
                                  <td className="p-3" key={j}>
                                    {row[h]}
                                  </td>
                                ))}
                  ...
                  <div className="mt-10 pt-8 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
                    ...
                    <div className="flex gap-4">
                      ...
                      <button
                        className="px-8 py-2.5 rounded-lg bg-primary text-white text-sm font-bold shadow-lg shadow-primary/25 hover:bg-primary/90 transition-all"
